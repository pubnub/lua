...
function pubnub.base(init)
    ...
    init.pnsdk          = 'PubNub-Lua-Corona/3.6.0'
...
function pubnub.new(init)
    ...
    function self:_request(args)
        ...
        local method = method = args.method or "GET"
        if args.body then params.body = args.body end

        request_id = network.request( args.url, method, function(event)
